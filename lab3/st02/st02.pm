...
my @MODULES =(
	...
	\&delete,
	\&save);
my @ElNames=('Name', 'Surname', 'Age', 'SS');
...
sub IntoDB{
	...
	$dbh->do('TRUNCATE TABLE sts');
	my $names = join(',',@ElNames);
	my $sth = $dbh->prepare("INSERT INTO sts(ID, $names) VALUES (?, ?, ?, ?, ?)");
	my $id = 1;
...
sub FromDB{
	...
	my $dbh = DBI->connect($dsn, $userid, $password, {RaiseError => 1, AutoCommit=>1}) or die $DBI::errstr;
	$dbh->do('SET CHARACTER SET cp1251');
	my $sth = $dbh->prepare ("SELECT * FROM sts;");
	$sth->execute();
	@DATABASE=();
...
sub edit {
	...
	print "<pre><FORM ACTION=$global->{selfurl} METHOD=POST><INPUT TYPE=hidden NAME=ElN value=$elnum><INPUT TYPE=hidden NAME=student value=$global->{st}>";
	my $str ="";
	foreach my $el(@ElNames) {
		if($el eq $ElNames[-1]){
			if ($DATABASE[$elnum-1]->{$el}==1) {print "Староста <INPUT Type=checkbox Name=\"SS\" Value=1 Checked>";}
			else {print "Староста <INPUT Type=checkbox Name=\"SS\" Value=1 unchecked>";}
		}else{
